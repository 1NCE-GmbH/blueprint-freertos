...
void RXFIFO_readMsgHeader_at_pos(const IPC_Handle_t *hipc, IPC_RxHeader_t *header, uint16_t pos)
{
  ...
  {
#if (DBG_IPC_RX_FIFO != 0U)
    PRINT_DBG("DBG RXFIFO_readMsgHeader: index_read = %d", hipc->RxQueue.index_read)
#endif /* DBG_IPC_RX_FIFO */
    ...
    PRINT_DBG("complete=%d size=%d", header->complete, header->size)
  }
}
