...
{
    IotLogInfo( "  /%u: Server object, instances:\r\n", object->objID);
    server_instance_t * serverInstance = (server_instance_t *)object->instanceList;
    while (serverInstance != NULL)
    {
        IotLogInfo( "    /%u/%u: instanceId: %u, shortServerId: %u, lifetime: %u, storing: %s, binding: %s",
        ...
        if(serverInstance->registrationPriorityOrder >= 0)
            IotLogInfo(", registrationPriorityOrder: %d", serverInstance->registrationPriorityOrder);
        if(serverInstance->initialRegistrationDelayTimer >= 0)
            IotLogInfo( ", initialRegistrationDelayTimer: %d", serverInstance->initialRegistrationDelayTimer);
        if(serverInstance->registrationFailureBlock >= 0)
            IotLogInfo(", registrationFailureBlock: %s",
                    serverInstance->registrationFailureBlock > 0 ? "true" : "false");
        if(serverInstance->bootstrapOnRegistrationFailure >= 0)
            IotLogInfo( ", bootstrapOnRegistrationFaulure: %s",
                    serverInstance->bootstrapOnRegistrationFailure > 0 ? "true" : "false");
        if(serverInstance->communicationRetryCount >= 0)
            IotLogInfo( ", communicationRetryCount: %d", serverInstance->communicationRetryCount);
        if(serverInstance->communicationRetryTimer >= 0)
            IotLogInfo( ", communicationRetryTimer: %d", serverInstance->communicationRetryTimer);
        if(serverInstance->communicationSequenceDelayTimer >= 0)
            IotLogInfo( ", communicationSequenceDelayTimer: %d", serverInstance->communicationSequenceDelayTimer);
        if(serverInstance->communicationSequenceRetryCount >= 0)
            IotLogInfo( ", communicationSequenceRetryCount: %d", serverInstance->communicationSequenceRetryCount);
#endif
        IotLogInfo(stdout, "\r\n");
